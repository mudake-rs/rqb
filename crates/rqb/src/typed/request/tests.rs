...
fn json_value_kinds_compile_to_typed_params() {
    ...
    let built = crate::typed::select(value_source())
        .request(request)
        .unwrap()
        .build()
        .unwrap();
...
fn additional_json_value_kinds_compile_to_typed_params() {
    ...
    let built = crate::typed::select(extra_value_source())
        .request(request)
        .unwrap()
        .build()
        .unwrap();
...
fn invalid_json_value_kinds_are_rejected_with_field_specific_expectations() {
    ...
    assert!(matches!(
        crate::typed::select(source()).request(bad_int).unwrap_err(),
        crate::Error::InvalidSearchValue {
            field,
    ...
    assert!(matches!(
        crate::typed::select(extra_value_source())
            .request(bad_numeric)
            .unwrap_err(),
        crate::Error::InvalidSearchValue {
            field,
...
fn malformed_uuid_json_value_is_rejected() {
    ...
    let err = crate::typed::select(value_source())
        .request(request)
        .unwrap_err();
